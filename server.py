...
def create_mcp_server(client: ComfyUIClient | None = None) -> FastMCP:
    ...
    async def upload_image(source: str) -> str:
        ...
        When chaining templates (e.g., generate → upscale), reference the prior output
        inline as `@{<ref>}` instead.
    ...
    async def run_template(
        ...
        Args:
            ...
            params: JSON string of parameter values, e.g. '{"输入文本": "hello"}'.
                ...
                exact `result://...` string from a prior result's `ref` field. Text outputs are
    ...
    async def run_templates(pipeline: str, timeout_per_step: float = 300) -> str:
        ...
        result's `ref` field. Text outputs are substituted as-is (free concatenation),
    ...
    def use_template() -> str:
        ...
        return (
            ...
            "### CRITICAL: Always Reference Prior Outputs Inline for Image Chaining\n\n"
            "When processing an image generated by a previous template (e.g., upscale, encrypt, img2img, style transfer), "
            "you **MUST** reference it inline as `@{<ref>}` to pass the image automatically. **NEVER** download and re-upload images manually.\n\n"
